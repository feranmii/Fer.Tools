...
public static class EditorSceneSwitcher
{
    ...
    public static readonly List<string> GeneralScenePaths = new();
    public static readonly List<string> LevelScenePaths = new();
    ...
    {
        // clear scenes 
        GeneralScenePaths.Clear();
        LevelScenePaths.Clear();

        // find all scenes in the Assets folder
        var generalSceneGuids = AssetDatabase.FindAssets("t:Scene", new[] { "Assets" });
        var levelSceneGuids = AssetDatabase.FindAssets("t:Scene", new[] { "Assets/Scenes" });

        foreach (var sceneGuid in generalSceneGuids.Concat(levelSceneGuids))
        {
            ...
            {
                GeneralScenePaths.Add(scenePath);
            }
        }
    }
}
...
public class SceneDropdown : EditorToolbarDropdown
{
    ...
    private void ToggleDropdown()
    {
        ...
        // Level Scenes
        menu.AddSeparator("");
        menu.AddDisabledItem(new GUIContent("Level Scenes"));
        foreach (var scenePath in EditorSceneSwitcher.LevelScenePaths)
        {
            AddMenuItemForScene(menu, scenePath);
        }

        // General Scenes
        menu.AddDisabledItem(new GUIContent("General Scenes"));
        foreach (var scenePath in EditorSceneSwitcher.GeneralScenePaths)
        {
            AddMenuItemForScene(menu, scenePath);
        }
    ...
    private void AddMenuItemForScene(GenericMenu menu, string scenePath)
    {
